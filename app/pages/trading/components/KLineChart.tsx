...
export function KLineChart({
  ...
  useEffect(() => {
    ...
    // 向前显示的时间（历史数据）- 显示1分钟
    const forwardDuration = Math.min(dataDuration, 1 * 60); // 向前显示已有数据或1分钟，取较小值
    
    // 向后显示的时间：如果有倒计时目标时间，确保能显示到目标时间；否则显示4分钟
    let backwardDuration = 4 * 60; // 默认4分钟
    if (countdownTargetTime !== null) {
      ...
      backwardDuration = Math.max(timeToTarget + 1 * 60, 4 * 60); // 至少显示到目标时间+1分钟，或4分钟
    }
